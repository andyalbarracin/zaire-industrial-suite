...
export default async function EditarVisitaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href={ROUTES.field.visita(id)} className="inline-flex items-center gap-1 text-sm text-(--zaire-text-muted) hover:text-zaire-blue mb-2">
          <ChevronLeft className="w-4 h-4" /> Volver al detalle
        </Link>
        <h1 className="text-2xl font-bold text-(--zaire-text)">Editar {visit.visit_number ?? "Visita"}</h1>
      </div>
      <VisitForm
        visit={visit}
